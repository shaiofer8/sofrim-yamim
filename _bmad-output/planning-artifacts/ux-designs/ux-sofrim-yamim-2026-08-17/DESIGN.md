...
**הוחלט (שי, 2026-08-17):** המוצר תומך **גם במצב בהיר וגם בכהה**, לפי העדפת המערכת (`prefers-color-scheme`) — לא כהה-בלבד. הפלטה הכהה (למעלה) נשארת ברירת המחדל התפיסתית של המותג ומוצגת ללא שינוי כשהמערכת במצב כהה; במצב בהיר מוצגת פלטה מקבילה (ר' Story 1.9) שנאמנה לאותה שפה (glass, גרדיאנט סגול-ורוד כעוגן), עם ערכי טקסט/ניגודיות משלה שנבדקו ל-WCAG AA. הגרדיאנט `accent-violet`→`accent-pink` עצמו אינו משתנה בין המצבים (משמש כמילוי-רקע קבוע מאחורי טקסט לבן-כפוי, לא כטקסט); מספר-הספירה (טקסט gradient-clip) משתמש בטוקנים ייעודיים (`--count-grad-1/2`) שכן משתנים בין המצבים כדי לשמור על ניגודיות.
...
- **טקסט/אייקונים על גבי הגרדיאנט (`accent-violet`→`accent-pink`) ישירות** (כותרת, FAB) — **חייבים להיות לבן מלא (`#ffffff`) עם משקל מודגש**, לא `text-primary`. `accent-pink` (`#ff5f8f`) לבדו לא עומד ב-4.5:1 מול `text-primary` (`#f4f1fb`, כמעט-לבן).
  **תיקון (שי/Claude, 2026-08-17, Story 1.4):** ההנחה המקורית כאן — שלבן-מלא לבדו עומד ב-4.5:1 "בכל נקודה על הגרדיאנט, כולל `accent-pink`" — התבררה כשגויה בחישוב בפועל: לבן על `accent-pink` הטהור הוא כ-**2.88:1 בלבד**, נכשל אפילו בסף המקל של טקסט-גדול (3:1). לבן על `accent-violet` הטהור הוא 4.35:1 — גם הוא לא עומד ב-4.5:1. **הפתרון שמומש:** שכבת-הצללה שחורה עדינה (30% אלפא) מוטמעת ב-`.app-header` מעל הגרדיאנט המותגי עצמו (שנשאר ללא שינוי בכל שאר השימושים — כפתורים, halo של ה-Hero Card, טוקני `--count-grad-1/2`) — מביאה את ניגודיות הלבן ל-**5.38:1 לפחות בכל נקודה**, כולל הקצה הכי בעייתי. ר' `style.css` (`.app-header`) לפרטים.
