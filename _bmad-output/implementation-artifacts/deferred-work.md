...
- source_spec: `_bmad-output/implementation-artifacts/spec-3-1-ad-banner.md`
  summary: `#ad-banner` uses a generic responsive AdSense `<ins data-ad-format="auto" data-full-width-responsive="true">` unit inside a `min-height:48px` fixed bottom bar. A real responsive ad isn't guaranteed to be exactly 48px tall, so `.event-list`'s matching `padding-bottom` (also hardcoded to 48px) is a best-effort estimate, not a guarantee the banner never covers the last row. Google's "Anchor ad" format exists specifically for this fixed-bottom-bar placement and handles sizing itself.
